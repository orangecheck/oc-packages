...
All notable changes to **`@orangecheck/react`** will be documented in this file.
...
- **`@orangecheck/sdk` `^1.4.0` → `^1.7.0`.** The range previously resolved
  ...
  (SECURITY.md §3, reason `stake_shared`). A `<Gate>` given an `identity` now follows those rules.

## [0.2.0] — 2026-09-03

### Fixed

- **`@orangecheck/sdk` `^0.1.3` → `^1.4.0`.** A `<Gate>` given an `address` never admitted anyone. Given an `attestationId` or `identity` it behaved correctly.
...
Initial public release. `<OcBadge>`, `<OcGate>`, `<OcChallengeButton>` React components.
...
[Unreleased]: https://github.com/orangecheck/oc-packages/compare/orangecheck-react-v0.1.3...HEAD
[0.1.3]: https://github.com/orangecheck/oc-packages/releases/tag/orangecheck-react-v0.1.3
