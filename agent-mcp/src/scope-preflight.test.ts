...
import { stampInvocation } from './index.js';
 ...
 */
describe('stampInvocation scope pre-flight', () => {
    const base = {
        agent: { address: AGENT, sign: sign() },
        invocation: { server: 'https://m.example.com', tool: 'search', arguments: { q: 'x' } },
    } as never;

    it('names encrypted scopes rather than blaming the scope string', async () => {
        const err = await stampInvocation({
            ...(base as object),
            delegation: { scopes_encrypted: SEALED },
            scopeExercised: 'mcp:invoke(server=https://m.example.com,tool=search)',
