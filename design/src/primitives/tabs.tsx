...
function TabsList({
  ...
  return (
    <TabsPrimitive.List
      data-slot="tabs-list"
      // A tab row is `flex` (nowrap), so a long set of triggers overflows
      // the page horizontally on narrow screens. Let the row scroll inside
      // itself instead — `overflow-x-auto` + `min-w-0`, scrollbar hidden so
      // it still reads as a clean tab strip. The negative margins keep the
      // triggers' -mb-px border overlap flush.
      className={cn(
