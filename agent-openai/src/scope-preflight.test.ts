...
import { stampFunctionCall } from './index.js';
 ...
 */
describe('stampFunctionCall scope pre-flight', () => {
    const base = {
    ...
    it('names encrypted scopes rather than blaming the scope string', async () => {
        const err = await stampFunctionCall({
            ...(base as object),
            delegation: { scopes_encrypted: SEALED },
            scopeExercised: 'openai:tool(name=search)',
